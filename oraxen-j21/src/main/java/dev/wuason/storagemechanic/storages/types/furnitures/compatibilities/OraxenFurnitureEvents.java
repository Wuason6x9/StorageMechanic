...
public class OraxenFurnitureEvents implements Listener {

    private final String ADAPTER_TYPE = "or:";
    private FurnitureStorageManager furnitureStorageManager;

    public OraxenFurnitureEvents(FurnitureStorageManager furnitureStorageManager) {
        this.furnitureStorageManager = furnitureStorageManager;
    }

    @EventHandler
    public void onFurnitureInteract(OraxenFurnitureInteractEvent event) {
        String adapterId = ADAPTER_TYPE + event.mechanic().getItemID();
        EventCancel eventCancel = new EventCancel();
        furnitureStorageManager.onFurnitureInteract(adapterId, event.player(), event.baseEntity(), event.itemInHand(), eventCancel);
