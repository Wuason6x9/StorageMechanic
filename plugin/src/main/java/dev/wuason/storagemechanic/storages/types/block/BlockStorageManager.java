...
public class BlockStorageManager implements Listener {
        ...
        if (Compatibilities.isOraxenLoaded()) {
            if (Compatibilities.isOraxenNew()) {
                try {
                    ...
                    Object oraxenEvents = oraxenEventsClass.getDeclaredConstructor(BlockStorageManager.class).newInstance(this);
    ...
    public void removeBlockStorage(String blockStorageId) {
        ...
        if (dataManager.getStorageManagerData().getBlockStorageManagerData().existBlockStorageData(blockStorageId))
            dataManager.getStorageManagerData().getBlockStorageManagerData().removeBlockStorageData(blockStorageId);
    }
    ...
    public BlockStorage getBlockStorage(String blockStorageId) {
        ...
        if (dataManager.getStorageManagerData().getBlockStorageManagerData().existBlockStorageData(blockStorageId))
            return loadBlockStorage(blockStorageId);
        return null;
    }
    ...
    public boolean isBlockStorageByBlock(Block block) {
        if (block.getChunk().getPersistentDataContainer().has(new NamespacedKey(core, "BlockStorage_" + block.getX() + "_" + block.getY() + "_" + block.getZ()), PersistentDataType.STRING))
            return true;
        if (block.getChunk().getPersistentDataContainer().has(new NamespacedKey(core, "BlockStorageShulker_" + block.getX() + "_" + block.getY() + "_" + block.getZ()), PersistentDataType.STRING))
            return true;
        return false;
    }
    ...
    public String getBlockStorageIDByBlock(Block block) {
        if (block.getChunk().getPersistentDataContainer().has(new NamespacedKey(core, "BlockStorage_" + block.getX() + "_" + block.getY() + "_" + block.getZ()), PersistentDataType.STRING))
            return block.getChunk().getPersistentDataContainer().get(new NamespacedKey(core, "BlockStorage_" + block.getX() + "_" + block.getY() + "_" + block.getZ()), PersistentDataType.STRING).split(":")[0];
        if (block.getChunk().getPersistentDataContainer().has(new NamespacedKey(core, "BlockStorageShulker_" + block.getX() + "_" + block.getY() + "_" + block.getZ()), PersistentDataType.STRING))
            return block.getChunk().getPersistentDataContainer().get(new NamespacedKey(core, "BlockStorageShulker_" + block.getX() + "_" + block.getY() + "_" + block.getZ()), PersistentDataType.STRING).split(":")[0];
        return null;
    }
    ...
    public BlockStorage getBlockStorageByNameSpace(NamespacedKey namespacedKey, Chunk chunk) {
        if (chunk.getPersistentDataContainer().has(namespacedKey, PersistentDataType.STRING))
            return getBlockStorage(chunk.getPersistentDataContainer().get(namespacedKey, PersistentDataType.STRING).split(":")[0]);
        return null;
    }

    public BlockStorageConfig getBlockStorageConfigByNameSpace(NamespacedKey namespacedKey, Chunk chunk) {
        if (chunk.getPersistentDataContainer().has(namespacedKey, PersistentDataType.STRING))
            return core.getManagers().getBlockStorageConfigManager().getBlockStorageConfig(chunk.getPersistentDataContainer().get(namespacedKey, PersistentDataType.STRING).split(":")[1]);
        return null;
    }
