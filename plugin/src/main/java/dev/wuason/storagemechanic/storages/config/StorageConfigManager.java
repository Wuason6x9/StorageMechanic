        ...
        for (File file : files) {
            ...
            if (sectionStorages != null) {
                for (Object key : sectionStorages.getKeys(false).toArray()) {
                    ...
                    if (storageInventoryType.equals(StorageInventoryTypeConfig.CHEST)) {
                        if (rows == 0 || rows > 6) {
                            AdventureUtils.sendMessagePluginConsole(core, "<red>Error loading Storage Config! storage_id: " + key + " in file: " + file.getName());
                            AdventureUtils.sendMessagePluginConsole(core, "<red>Error: The rows are empty or the number is greater than 6");
                            continue;
                        }
                    }
                    ...
                    if (sectionSounds != null) {
                        for (Object soundKey : sectionSounds.getKeys(false).toArray()) {
                            ...
                            try {
                                soundType = StorageSoundConfig.Type.valueOf(sectionSound.getString("type", ".").toUpperCase());
                            } catch (IllegalArgumentException a) {
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error loading Storage sound Config! storage_id: " + key + " sound_id: " + soundKey + " in file: " + file.getName());
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error: soundType is null or invalid");
                                continue;
                            }
                            ...
                            if (sound.equals(".")) {
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error loading Storage sound Config! storage_id: " + key + " sound_id: " + soundKey + " in file: " + file.getName());
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error: sound is null or invalid");
                                continue;
                            }
                    ...
                    if (sectionInterfaces != null) {
                        for (Object interfaceItemKey : sectionInterfaces.getKeys(false).toArray()) {
                            ...
                            if (!checkSlots(interfaceItemSlots)) {
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error loading Storage " + "interfaceItem" + " item Config! storage_id: " + key + " " + "interfaceItem" + "Item_id: " + interfaceItemKey + " in file: " + file.getName());
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error: " + "interfaceItem" + " Some slot is wrong the range of slots is: 0 - 63");
                                continue;
                            }
                            String interfaceItem = sectionInterface.getString("item", ".");
                            if (interfaceItem.equals(".")) {
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error loading Storage interfaceItem Config! storage_id: " + key + " interfaceItem_id: " + interfaceItemKey + " in file: " + file.getName());
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error: interfaceItem is null or invalid");
                                continue;
                            }
                            if (!core.getManagers().getItemInterfaceManager().existsItemInterface(interfaceItem)) {
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error loading Storage interfaceItem Config! storage_id: " + key + " interfaceItem_id: " + interfaceItemKey + " in file: " + file.getName());
                                AdventureUtils.sendMessagePluginConsole(core, "<red>Error: interfaceItem is null or invalid");
                                continue;
                            }
                            ...
                            for (int page : interfaceItemPages) {
                                for (int slot : interfaceItemSlots) {
                                    if (!storageInterfacesConfigs.containsKey(page))
                                        storageInterfacesConfigs.put(page, new HashMap<>());
                        ...
                        for (String stageKey : sectionStages.getKeys(false)) {
                            ...
                            if (sectionStageInterfaces != null) {
                                for (String interfaceStageItemKey : sectionStageInterfaces.getKeys(false)) {
                                    ...
                                    ArrayList<Integer> interfaceItemSlots = StorageUtils.configFill(sectionInterface.getStringList("slots"));
                                    if (!checkSlots(interfaceItemSlots)) {
                                        AdventureUtils.sendMessagePluginConsole(core, "<red>Error loading Storage " + "interfaceItem" + " item Config! in Stage_id: " + stageKey + " storage_id: " + key + " " + "interfaceItem" + "Item_id: " + interfaceStageItemKey + " in file: " + file.getName());
                                        AdventureUtils.sendMessagePluginConsole(core, "<red>Error: " + "interfaceItem" + " Some slot is wrong the range of slots is: 0 - 63");
                                        continue;
                                    }
                                    String interfaceItem = sectionInterface.getString("item", ".");
                                    if (interfaceItem.equals(".")) {
                                        AdventureUtils.sendMessagePluginConsole(core, "<red>Error loading Storage interfaceItem Config! in Stage_id: " + stageKey + " storage_id: " + key + " interfaceItem_id: " + interfaceStageItemKey + " in file: " + file.getName());
                                        AdventureUtils.sendMessagePluginConsole(core, "<red>Error: interfaceItem is null or invalid");
                                        continue;
                                    }
                                    if (!core.getManagers().getItemInterfaceManager().existsItemInterface(interfaceItem)) {
                                    ...
                                    for (int page : interfaceItemPages) {
                                        for (int slot : interfaceItemSlots) {
                                            if (!storageInterfacesStageConfigs.containsKey(page))
                                                storageInterfacesStageConfigs.put(page, new HashMap<>());
